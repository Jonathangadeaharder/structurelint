// Package test provides test validation rules.
package test
...
func (r *TestAdjacencyRule) Check(files []walker.FileInfo, dirs map[string]*walker.DirInfo) []rules.Violation {
...
func (r *TestAdjacencyRule) checkAdjacentPattern(files []walker.FileInfo) []rules.Violation {
	var violations []rules.Violation
...
func (r *TestAdjacencyRule) checkAdjacentFile(file walker.FileInfo, filesByDir map[string][]walker.FileInfo) *rules.Violation {
	if file.IsDir {
		return nil
	}
	...
	hasIgnoreDirective, reason := rules.ShouldIgnoreFile(file, r.Name())
...
func (r *TestAdjacencyRule) checkSeparatePattern(files []walker.FileInfo) []rules.Violation {
	var violations []rules.Violation
...
func (r *TestAdjacencyRule) checkSeparateFile(sourcePath string, sourceFile walker.FileInfo, testFiles map[string]bool) *rules.Violation {
	// Check if file has directive to ignore this rule
	if hasIgnore, _ := rules.ShouldIgnoreFile(sourceFile, r.Name()); hasIgnore {
		return nil
	}
